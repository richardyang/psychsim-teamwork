...
def create_base(world):
    ...
    tree = makeTree(setToConstantMatrix(stateKey('Distractor', 'deployed'), True))
    world.setDynamics(stateKey('Distractor', 'deployed'), action, tree)
    ...
    base.setReward(minimizeDifference(stateKey('Distractor', 'x'), stateKey('Enemy0', 'x')), BASE[0])
    base.setReward(minimizeDifference(stateKey('Distractor', 'y'), stateKey('Enemy0', 'y')), BASE[0])
    base.setReward(minimizeFeature(stateKey('Distractor', 'cost')), BASE[1])
...
def create_friendly_agents(world):
    for index in range(0, F_ACTORS):
        ...
        tree = {'if': equalFeatureRow(stateKey(actor.name, 'x'), stateKey(actor.name, 'goal_x')),
                True: {'if': equalFeatureRow(stateKey(actor.name, 'y'), stateKey(actor.name, 'goal_y')), True: True,
                       False: False},
                False: False}
...
def create_distract_agents(world):
    ...
    actor.setReward(minimizeDifference(stateKey('Actor0', 'x'), stateKey('Enemy0', 'x')), DISTRACTOR)
    actor.setReward(minimizeDifference(stateKey('Actor0', 'y'), stateKey('Enemy0', 'y')), DISTRACTOR)
...
def create_enemy_agents(world):
    for index in range(0, E_ACTORS):
        ...
        #
        # actor.setReward(minimizeDifference(stateKey(actor.name, 'x'), stateKey('Distractor', 'x')), ENEMY[1])
        # actor.setReward(minimizeDifference(stateKey(actor.name, 'y'), stateKey('Distractor', 'y')), ENEMY[1])
...
distractor_sprite = pyglet.sprite.Sprite(img=distractor_image, x=0 * 32, y=0 * 32)
